...
const Registro = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.nombre.trim() && (formData.nombre.length < 2 || !/^[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ\s]+$/.test(formData.nombre))) {
      validationErrors.nombre = "Nombre inválido";
    }

    validateRequiredField('apellido', 'Este campo es obligatorio');
    if (formData.apellido.trim() && (formData.apellido.length < 2 || !/^[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ\s]+$/.test(formData.apellido))) {
      validationErrors.apellido = "Apellido inválido";
    }

    validateRequiredField('email', 'Este campo es obligatorio');
    if (formData.email.trim() && !/^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i.test(formData.email)) {
    ...
    validateRequiredField('nacimiento', 'Por favor ingrese su fecha de nacimiento');
    if (formData.nacimiento) {
      ...
      if (birthDate > today) {
        validationErrors.nacimiento = "Por favor ingrese una fecha de nacimiento valida";
      }
    }
      ...
      <Form onSubmit={handleSubmit}>
        <Row className="mb-3">
          ...
          <Form.Group as={Col}>
            ...
            <Form.Control onChange={handleChange} name="apellido" type="text" placeholder="Apellidos" />
            {errors.apellido && <span className="error-message">{errors.apellido}</span>}
          </Form.Group>
        </Row>
        <Form.Group className="mb-3">
          <Form.Label>Correo</Form.Label>
          <Form.Control onChange={handleChange} name="email" type="correo" placeholder="Correo" />
        ...
        <Form.Group className="mb-3">
          ...
          {errors.preferencia && <span className="error-message">{errors.preferencia}</span>}
        </Form.Group>
        ...
        <Form.Group className="mb-3">
          ...
          <Form.Text className="text-muted">
            Consigue una recompensa de Xclusive Member por tu cumpleaños. <br />
          </Form.Text>
          {errors.nacimiento && <span className="error-message">{errors.nacimiento}</span>}
        </Form.Group>
