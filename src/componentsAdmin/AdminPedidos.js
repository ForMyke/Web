...
const AdminPedidos = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost/backend/api/pedidos.php").then((response) => {
                  ...
                  <React.Fragment key={venta_id}>
                    {grupoPedidos.map((pedido, index) => (
                      <tr key={pedido.id}>
                        {index === 0 && (
                          <>
                            <td rowSpan={grupoPedidos.length}>{pedido.email}</td>
                            <td rowSpan={grupoPedidos.length}>{pedido.fecha}</td>
